...
public:
	vector<pair<int, int>> getSkyline(vector<vector<int>>& buildings) {
		...
		for(int i = 0; i < segments.size(); ++i) {
			//cout << segments[i].begin << " " << segments[i].end << endl;
		}
...
private:
	...
	void getHeights() {
		...
		while (!q.empty()) {
			...
			{
				cout << "H " << tree[idx].nodel << " " << tree[idx].nodeh << endl;
...
int main() {
	vector<vector<int>> vv = { {2, 9, 10}, {3, 7, 15}, {5, 12, 12}, {15, 20, 10}, {19, 24, 8}, {122222222, 122222223, 3} };
	Solution sol;
	sol.getSkyline(vv);

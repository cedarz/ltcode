...
public:
	...
	bool isScramble(string s1, string s2) {
		...
		for(int part = 1; part < len; ++ part) {
			...
			bool left = false, right = false;
			if(t1 == t2) {
				return  isScramble(s1.substr(0, part), s2.substr(0, part)) && isScramble(s1.substr(part, len - part), s2.substr(part, len - part));
			} else {
				return isScramble(s1.substr(0, len - part), s2.substr(part, len - part)) && isScramble(s1.substr(len - part, part), s2.substr(0, part));
			}
		}
		return false;
	}
